...
#[derive(Debug)]
pub struct MutexBackedHistogram {
    // Mutex-secured, Vec-backed value store for the histogram.
...
impl MutexBackedHistogram {
    ...
    pub fn get(&self, bin: usize) -> usize {
        // SAFETY: MutexGuard is dropped at the end of the scope automatically.
        let values = self.values.lock().unwrap();
        *values.get(bin).unwrap_or_else(|| {
            panic!(
    ...
    pub fn sum(&self) -> usize {
        // SAFETY: MutexGuard is dropped at the end of the scope.
        let values = self.values.lock().unwrap();
        values.iter().sum()
    }

    /// Simply returns the values as a Vec.
    ...
    pub fn values_normalized(&self) -> Vec<f64> {
        ...
        // SAFETY: MutexGuard is dropped at the end of the scope.
        let values = self.values.lock().unwrap();
        values.iter().map(|x| *x as f64 / total).collect()
    }
}

impl Default for MutexBackedHistogram {
    fn default() -> Self {
...
impl MutexBackedHistogram {
    pub fn clone_nonmutex(&self) -> Histogram {
        let values = self.values.lock().unwrap();
        Histogram {
            values: values.clone(),
...
mod tests {
    ...
    pub fn test_initialization() {
        let s = MutexBackedHistogram::zero_based_with_capacity(100);
        assert_eq!(s.len(), 101);
    ...
    pub fn test_valid_incremements_and_mean_median() {
        let mut s = MutexBackedHistogram::zero_based_with_capacity(100);
    ...
    pub fn test_median_on_empty_histogram() {
        let s = MutexBackedHistogram::zero_based_with_capacity(5000);
        assert!(s.median().is_none());
    }

    #[test]
    pub fn test_median_extensively() {
        let mut s = MutexBackedHistogram::zero_based_with_capacity(5000);
    ...
    pub fn test_default_is_one_hundred_zero_based() {
        ...
        let default = MutexBackedHistogram::default();
    ...
    #[test]
    pub fn test_values() {
        let mut histogram = MutexBackedHistogram::zero_based_with_capacity(3);
