...
use crate::lib::{
    qc::{
        coverage::CoverageFacet, features::GenomicFeaturesFacet, gc_content::GCContentFacet,
...
pub fn get_sequence_based_qc_facets<'a>(
    ...
    // Default facets that are loaded within the qc subcommand.
    let facets: Vec<Box<dyn SequenceBasedQualityCheckFacet>> =
...
async fn app(
    ...
    let mut sequence_facets = get_sequence_based_qc_facets()?;
    info!("");
    ...
    for (name, seq) in header.reference_sequences() {
        ...
        while let Some(record) = query.try_next().await.unwrap() {
            for facet in &mut sequence_facets {
                if facet.supports_sequence_name(name) {
                    facet.process(seq, &record).unwrap();
                }
            }
        ...
        for facet in &mut sequence_facets {
            if facet.supports_sequence_name(name) {
                facet.summarize_seq(seq).unwrap();
            }
        }
    }
